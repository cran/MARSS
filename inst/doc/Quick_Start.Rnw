...
Each model parameter matrix, $\BB_t$, $\UU_t$, $\QQ_t$, $\ZZ_t$, $\AA_t$, and $\RR_t$, is written as a time-varying linear model, $\ff_t+\DD_t\mm$, where $\ff$ and $\DD$ are fully-known (not estimated and no missing values) and $\mm$ is a column vector of the estimated elements of the parameter matrix:
\begin{gather*}
...
The internal MARSS model specification (marssm objects) is a list with the $\ff_t$ and $\DD_t$ matrices for each parameter.   
