...
print.marssMLE <- function (x, digits = max(3, getOption("digits")-4), ...) 
    {
      ...
      if(("conv.test.slope.tol" %in% names(alldefaults[[x$method]])) & is.null(x$control$abstol)) 
            cat(paste("Convergence test: log-log plot with slope tol of", x$control$conv.test.slope.tol, "\n"))
      if(("conv.test.slope.tol" %in% names(alldefaults[[x$method]])) & !is.null(x$control$abstol))
            cat(paste("Convergence test: delta logLik with abstol of", x$control$abstol, "\n"))
	    if (x$convergence==0){
