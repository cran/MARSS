...
##############################################################################################################################################
print.marssMLE <- function (x, digits = max(3, getOption("digits")-4), ..., what="fit", form=NULL, silent=TRUE) {
#First make sure specified equation form has a corresponding function to do the conversion to marssm object
  ...
  print.fun = paste("print_",form,sep="")
  ...
  paramvector=MARSSvectorizeparam(x)

	cmat = as.matrix(paramvector)
	colnames(cmat) = "Estimate"
	if(!is.null(x[["par.se"]])) {
	  cmat = cbind(cmat, MARSSvectorizeparam(x,what="par.se"))
	  colnames(cmat) = c("ML.Est", "Std.Err")
	}

  if(!is.null(x[["par.lowCI"]]) & !is.null(x[["par.upCI"]])) {  
	  cmat = cbind(cmat, MARSSvectorizeparam(x,what="par.lowCI"))
	  colnames(cmat)[dim(cmat)[2]]="low.CI"
	  cmat = cbind(cmat, MARSSvectorizeparam(x,what="par.upCI"))
	  colnames(cmat)[dim(cmat)[2]]="up.CI"  
	}
	if(!is.null(x[["par.bias"]])) {
    par.bias = MARSSvectorizeparam(x,what="par.bias") 
  ...
  if(!is.null(x[["errors"]])) cat(x$errors)
    ...
    if(what=="model"){
      print.fun=paste("print.model.",form,sep="")
    ...
    if(what%in%c("par.se","par.bias","par.upCI","par.lowCI")){
      if(is.null(x[[what]])){        
        if(what!="par.bias"){ 
          ...
          x=MARSSparamCIs(x,method="parametric")
        }
      } 
      if(!silent){ cat(paste("Estimated ",what,":\n")); print(MARSSvectorizeparam(x,what=what)); cat("\n") }
      return.obj[[what]]=x[[what]]
    }
    if(what=="paramvector"){
      paramvector = MARSSvectorizeparam(x)
    ...
    if(what=="kfs"){
      ...
      if(x$fun.kf=="MARSSkfas"){
        tmp=MARSSkfss(x)   #MARSSkfas doesn't return Innov or Sigma
        kf$Innov=tmp$Innov
        kf$Sigma=tmp$Sigma
      }
    ...
    if(what=="Ey"){
      Ey = MARSShatyt(x)  #MARSSkf since it outputs more info
    ...
    if(what=="start" | what=="inits"){
      ...
      if(!silent){ print(MARSSvectorizeparam(x,what="start")); ; cat("\n") }
      return.obj[[what]]=x$start
    }
    fixed=x$model$fixed
    free=x$model$free
    #if there is info on the parameter dimensions in the model list, use it
    if(!is.null(x[["model"]][["model.dims"]])){ par.dims=x$model$model.dims }else{ par.dim=NULL }
    if(what %in% names(fixed)){
      par.dim=par.dims[[what]]
      the.par = parmat(x,what,t=1:max(dim(fixed[[what]])[3],dim(free[[what]])[3]), dims=par.dim)[[what]]
