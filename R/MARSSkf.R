...
MARSSkf = function( MLEobj ) {
    ...
    for(i in 1:(TT-1)) {
      ...
      if(debugkf!=-1 & any(diag.Vtt1==0) ) {  
        ...
        if(!Q0s && (init.state=="x00" || (init.state=="x10" && t>1)) ){
          return(list(ok=FALSE, errors=paste("Stopped in MARSSkf: soln became unstable when zeros appeared on the diagonal of Vtt1 at t=",t,".\n") ) )
        }
      }
    ...
    if(init.state=="x00") { #Shumway and Stoffer treatment of initial conditions; LAM and pi defined for x_0
      ...
      if(debugkf!=-1 & any(diag.Vtt1==0) ) {  
        ...
        Q0s=identical(which(diag.Q==0),which(diag.Vtt1==0))
        if(!Q0s && (init.state=="x00" || (init.state=="x10" && t>1)) ){
          return(list(ok=FALSE, errors=paste("Stopped in MARSSkf: soln became unstable when zeros appeared on the diagonal of Vtt1 at =1.\n") ) )
        }
      }
      if(m==1){ Vinv=pcholinv(matrix(Vtt1[,,t],1,1))  #pcholinv doesn't like vectors
      }else{
        Vinv=pcholinv(Vtt1[,,t])
        Vinv = symm(Vinv)  #to enforce symmetry after chol2inv call
      }
