...
nr=dim(x)[1]
for(i in 1:nr){
   block = which(!sapply(tmpx[i,],identical,0))
   if(any(sapply(tmpx[block,block],identical,0)) ) return(FALSE)
   notblock = which(sapply(tmpx[i,],identical,0))
   if(!all(sapply(tmpx[notblock,block],identical,0))) return(FALSE)
   if(!all(sapply(tmpx[block,notblock],identical,0))) return(FALSE)
   }
...
for(i in 1:nr){
   block = which(!sapply(tmpx[1,],identical,0)) #1 since I keep shrinking the matrix
   if(!is.equaltri(tmpx[block,block]) ) return(FALSE)
   notblock = which(sapply(tmpx[1,],identical,0))
...
tmpx = x
for(i in 1:nr){
   block = which(!sapply(tmpx[1,],identical,0))
   if(any(sapply(tmpx[block,block],is.numeric))) return(FALSE) #the blocks need to be all estimated
   if(!is.list(tmpx)){ tmp = unique(vec(tmpx[block,block]))
   }else{ tmp = unique(unlist(tmpx[block,block])) }
   dimblock = length(block)
   if(!is.diagonal(convert.model.mat(tmpx[block,block])$free)) return(FALSE) #not unconstrained
   notblock = which(sapply(tmpx[1,],identical,0))
   if(length(notblock)==0) break
   tmpx=tmpx[notblock,notblock]
   dim(tmpx) = c(length(notblock),length(notblock))
...
#returns fixed and free matrices that are 3D as required for a marssm model
...
fixed.free.to.formula=function(fixed,free,dim){ #dim is the dims of the outputed list matrix
#this will take a 3D or 2D 
 ...
 if(length(dim(fixed))==3)
    if(dim(fixed)[3] != 1){ stop("fixed.free.to.formula: if fixed is 3D, dim 3 must be length 1")
    }else{ fixed=matrix(fixed,dim(fixed)[1],dim(fixed)[2]) }
 ...
 if(length(dim(free))==3)
    if(dim(free)[3] != 1){ stop("fixed.free.to.formula: if free is 3D, dim 3 must be length 1")
    }else{ free=sub3D(free,t=1) }
  #turns a time invariant fixed/free pair to a list matrix describing that MARSS parameter structure
  model=array(list(0),dim=dim(fixed)) #this will be 2D since fixed was changed to 2D even if passed in as 3D
  for(i in 1:dim(fixed)[1]){
    if(all(free[i,]==0) | dim(free)[2]==0){ model[i,1]=fixed[i,1]
    }else{
     if(fixed[i,1]==0) tmp=c() else tmp=c(as.character(fixed[i,1]))
     colnames.free=colnames(free)
     if(is.null(colnames(free))) colnames.free=as.character(1:dim(free)[2])
     for(j in 1:dim(free)[2]){
       if(free[i,j]!=0){
         if(is.null(tmp) & free[i,j]==1){ tmp=colnames.free[j]
         }else{ tmp=c(tmp,"+",as.character(free[i,j]),"*",colnames.free[j]) }
       }
     }
     if(tmp[1]=="+") tmp=tmp[2:length(tmp)]
     model[i,1]=paste(tmp,collapse="")
    }
  }
  return(unvec(model,dim=dim))
} 
