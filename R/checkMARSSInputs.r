...
#MARSS = function(y, inits=NULL, model=NULL, miss.value=NA, method = "kem", form = "marxss", fit=TRUE,  silent = FALSE, control = NULL, MCbounds = NULL ) 
#   checks y, duplication in inits, miss.value, method, form, fit, silent
...
# the user might reset these in their MARSS.form function
  req.args = c("inits", "MCbounds", "miss.value", "control")  #needed but not necessarily speced by user
...
#check that miss.value is numeric or NA; make sure R doesn't think miss.value=NA is logical
  if(is.na(MARSS.inputs$miss.value)) MARSS.inputs$miss.value = as.numeric(MARSS.inputs$miss.value)
  if( !(length(MARSS.inputs$miss.value)==1) || !(is.na(MARSS.inputs$miss.value) || is.numeric(MARSS.inputs$miss.value))){
    problem=TRUE
